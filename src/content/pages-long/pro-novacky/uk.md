---
title: "Я вперше"
description: "Практичний путівник для новачка — що таке подія, що таке LARP, вибір армії, костюм, правила, реєстрація, прибуття та вихідні в грі."
breadcrumb: "Я вперше"
hero_subtitle: "Ласкаво просимо в Середзем'я. Тут починається твоя перша виправа — крок за кроком від першого знайомства з подією до суботньої битви."
last_updated: "30 квітня 2026"
toc_title: "Шлях новачка"
toc_aria: "Шлях новачка"
brief:
  h2: "Найважливіше за 60 секунд"
  items:
    - "Володар Перснів — це вікендова LARP-битва, натхнена Середзем'ям."
    - "Головна битва відбувається в суботу, а програма починається вже в четвер і п'ятницю."
    - "У головній грі можуть брати участь гравці від 12 років."
    - "Учасникам до 18 років потрібна згода законного представника."
    - "Обери сторону та армію."
    - "Підготуй костюм, взуття, рукавиці, воду та спорядження для табору."
    - "Якщо хочеш битися, тобі потрібна безпечна м'яка зброя."
    - "Прочитай правила — бій, ураження, зброя та безпека."
    - "Зареєструйся та сплати реєстраційний внесок упродовж 10 днів."
    - "Після прибуття пройдеш реєстрацію та перевірку зброї/костюма."
chapters:
  - id: "co-je"
    label: "1. Що таке Пан Перснів"
    icon: "lucide:tent"
  - id: "co-je-larp"
    label: "2. Що таке ЛАРП"
    icon: "lucide:drama"
  - id: "vhodnost"
    label: "3. Чи підходить мені цей захід?"
    icon: "lucide:user-check"
  - id: "vek"
    label: "4. Вік і участь"
    icon: "lucide:cake"
  - id: "vyber-frakce"
    label: "5. Обери сторону та армію"
    icon: "lucide:flag"
  - id: "kostym"
    label: "6. Костюм і спорядження"
    icon: "lucide:shirt"
  - id: "pravidla"
    label: "7. Правила і безпека"
    icon: "lucide:shield-check"
  - id: "registrace"
    label: "8. Реєстрація і оплата"
    icon: "lucide:credit-card"
  - id: "prijezd"
    label: "9. Прибуття і перший день"
    icon: "lucide:map-pin"
  - id: "vikend"
    label: "10. Вікенд у грі"
    icon: "lucide:calendar-days"
  - id: "slovnicek"
    label: "Словничок"
    icon: "lucide:book-marked"
  - id: "kam-dal"
    label: "Куди далі"
    icon: "lucide:arrow-right"
  - id: "cteni"
    label: "Рекомендоване читання"
    icon: "lucide:book-open"
---

## Крок 1 — Що таке Володар Перснів {#co-je}

Володар Перснів — це велика багатоденна LARP-битва, натхнена світом Середзем'я. Гравців ділять на дві основні **сторони** та окремі **армії**. Кожна армія має власний стиль, кольори, костюми, атмосферу, місце в історії та спосіб, у який вступає в гру.

Ідеться не лише про суботню битву. Цілий вікенд починається раніше: прибуттям, встановленням табору, реєстрацією, знайомствами з іншими, п'ятничною грою в таборі, ярмарком, аренами, дитячою грою, вечірнім життям у таборі та спільною атмосферою. **Головна гра відбувається в суботу** — саме тоді армії стикаються і весь рік досягає кульмінації.

<div class="newcomer-info-grid">
  <section class="newcomer-info-card" aria-labelledby="newcomer-for-title">
    <h3 id="newcomer-for-title" class="newcomer-info-card-title">
      <span aria-hidden="true">◆</span>
      Подія для людей, які хочуть:
    </h3>
    <ul>
      <li>пережити фентезійні вихідні на природі,</li>
      <li>одягнути костюм і вступити в роль,</li>
      <li>битися безпечною м'якою зброєю,</li>
      <li>бути частиною великої армії,</li>
      <li>зустріти спільноту гравців з усієї країни,</li>
      <li>насолодитися табірним життям і вечірніми вогнищами.</li>
    </ul>
  </section>

  <section class="newcomer-info-card" aria-labelledby="newcomer-expect-title">
    <h3 id="newcomer-expect-title" class="newcomer-info-card-title">
      <span aria-hidden="true">◆</span>
      Що тебе чекає:
    </h3>
    <ul>
      <li>табір і вечірнє вогнище,</li>
      <li>ярмарок, арени та мікроквести,</li>
      <li>п'ятнична гра в таборі та вечірня програма,</li>
      <li>суботня головна гра і битва,</li>
      <li>недільне збирання та прощання.</li>
    </ul>
  </section>
</div>

*Якщо боїшся, що нікого не знаєш, не хвилюйся. На таких подіях побратимства народжуються швидше, ніж хобіт встигне сказати „другий сніданок".*

## Крок 2 — Що таке LARP {#co-je-larp}

LARP — це гра наживо. Не використовуєш фігурку на столі чи персонажа на моніторі — на якийсь час сам стаєш персонажем.

На Володарі Перснів це означає, що обираєш армію, готуєш костюм, береш спорядження, прибуваєш у табір і долучаєшся до спільної гри. Хтось приходить битися, хтось — як небойовий супровід, хтось фотографує, хтось допомагає, хтось приходить з дитиною. Усі разом створюємо атмосферу Середзем'я.

**LARP — це:** гра у світі з власними правилами, костюм, роль та атмосфера, фейр-плей і повага, спільне переживання.

**LARP — це не:** справжній бій, спортивне змагання заради перемоги, симуляція війни, перегони „за будь-яку ціну".

**Правило для новачка:** якщо не впевнений — питай. Якщо не впевнений у бою — краще визнай влучання. **Якщо щось скаже організатор — це остаточно.**

## Крок 3 — Чи ця подія для мене? {#vhodnost}

Так, якщо хочеш пережити вихідні у фентезійному світі, битися в битві, підготувати костюм, познайомитися з новими людьми, грати за одну з армій, допомогти з атмосферою і поза боєм або приїхати як супровід, фотограф, помічник, батько чи продавець. Не обов'язково бути досвідченим LARPером — головне прочитати основи, не недооцінювати підготовку і не боятися спілкуватися.

**Можливі ролі:**

- **Хочу битися** — обереш сторону та армію, підготуєш костюм, безпечну зброю і долучишся до головної гри. Це найпоширеніший шлях.
- **Небойовий супровід** — ти в грі і в костюмі, але не б'єшся. Бард, літописець, постачальник, супровід воїна, табірний персонаж.
- **Я з дитиною** — для молодших дітей маємо окрему дитячу гру. Старші за умови виконання вимог можуть брати участь у головній грі.
- **Хочу фотографувати чи знімати** — фотографи й оператори бажані, але і для них діє вимога носити костюм.
- **Хочу допомогти** — помічники можуть долучитися до підготовки, реєстрації, програми чи прибирання. Домовимося про знижку, звільнення від внеску чи харчування.
- **Хочу продавати / пропонувати послуги** — продавці прибувають з тематичним асортиментом. Гастро потрібно домовляти заздалегідь через шинок U Zeleného draka.

## Крок 4 — Вік і участь {#vek}

Головна гра і суботня битва призначені для учасників від **12 років**. Для молодших дітей готуємо окрему **дитячу гру**, яка відокремлена від головної битви і пристосована до віку маленьких пригодників.

<div class="newcomer-info-grid">
  <section class="newcomer-info-card" aria-labelledby="age-main-title">
    <h3 id="age-main-title" class="newcomer-info-card-title">
      <span aria-hidden="true">◆</span>
      Головна гра (12+):
    </h3>
    <ul>
      <li>від 12 років,</li>
      <li>до 18 років — згода законного представника,</li>
      <li>до 15 років — лише в супроводі особи старшої за 18 років,</li>
      <li>хто відповідає за дитину, має бути письмово чітко зазначено.</li>
    </ul>
  </section>

  <section class="newcomer-info-card" aria-labelledby="age-kids-title">
    <h3 id="age-kids-title" class="newcomer-info-card-title">
      <span aria-hidden="true">◆</span>
      Дитяча гра:
    </h3>
    <ul>
      <li>передусім для дітей 5–10 років,</li>
      <li>відокремлена від головної битви,</li>
      <li>пристосована до віку, безпеки і можливостей дітей,</li>
      <li>веде її команда організаторів, спеціалізована на дитячій програмі.</li>
    </ul>
  </section>
</div>

## Крок 5 — Обери сторону та армію {#vyber-frakce}

Подія має дві основні сторони: **Вільні народи Середзем'я** (Ґондор, Рохан, Ельфи, Гноми, Гобіти) і **Сили Темного володаря** (Орки, Урук-гаї, Гарад, Умбар, Гірські люди). Сторону не мусиш обирати одразу — спочатку перегляни армії, дізнайся, яка тобі пасує за кольором, атмосферою і стилем гри, і тоді виріши.

Кожна армія має **володаря / командира**, який тримає групу разом і спілкується з організаторами. Після реєстрації приєднаєшся до армії і в її межах підготуєш свого персонажа.

Деталі, кольори, типовий костюм, рекомендації для новачків і приклади знайдеш на сторінці [Армії та сторони](/uk/frakce/).

## Крок 6 — Костюм і спорядження {#kostym}

Костюм обов'язковий протягом усієї події. Оцінка костюма **визначає кількість життів** у головній битві (2–4 життя). Важлива атмосфера — костюм має пасувати армії, типу персонажа й атмосфері гри.

**Для новачка вистачить:**

- проста туніка чи сорочка в кольорі армії,
- штани / штанини,
- пояс із дрібницями (сумка, пляшка, торбинка),
- взуття, замасковане як історизуюче (обмотане тканиною, постоли, високі чоботи),
- простий головний убір або капюшон,
- рукавиці (захищають руки під час бою),
- тематично відповідні аксесуари.

**Спорядження для табору:**

- намет,
- спальний мішок і килимок / матрац,
- власна їжа і напої (або скористаєшся шинком U Zeleného draka),
- ліхтарик / лампа,
- ліки, гігієна, крем від сонця,
- дощовик, теплий одяг (вечори холодні).

**Якщо хочеш битися:**

- безпечна м'яка зброя, що пройде перевірку,
- щит (опціонально),
- ідеально — і проста броня або стьобанка,
- опціонально рекомендуємо захист голови — шолом, батват, шкіряний шолом тощо.

## Крок 7 — Правила і безпека {#pravidla}

Правила описують, **як проходить бій, як визнаються ураження, скільки маєш життів, що означає оживлення, які зброї дозволені і як поводитися в таборі**. Перед подією прочитай їх уважно, особливо якщо це твоя перша битва.

**Ключові правила:**

- Безпека на першому місці.
- Що скаже організатор — те остаточно.
- Грається фейр-плей.
- Б'ються по великих зонах ураження (тулуб, руки від тулуба до зап'ястя, ноги від тулуба до щиколоток).
- Влучання зброєю = 1 життя, влучання стрілою = 2 життя.
- Зброя має пройти схвалення.
- Участь під впливом алкоголю заборонена.
- Костюм обов'язковий протягом усієї події.

Деталі знайдеш на сторінці [Правила і безпека](/uk/pravidla/).

## Крок 8 — Реєстрація і оплата {#registrace}

Реєстрація відбувається онлайн через систему **Registračka.cz**. Триває близько 10 хвилин. Обери тип участі, армію і заповни дані.

**Після реєстрації:**

- Упродовж 10 днів отримаєш платіжні вказівки.
- З оплатою бронювання стає зобов'язувальним.
- Після оплати отримаєш підсумок.
- Остаточні вказівки і GPS табору надсилаємо зареєстрованим перед подією.

Деталі реєстрації, розмір внеску, знижки для родин і умови скасування знайдеш на сторінці [Реєстрація](/uk/registrace/).

> **Якщо реєстраційний внесок є для тебе перешкодою,** напиши нам. Домовимося про індивідуальне рішення, знижку чи дотацію. Більше — на сторінці [Коли реєстраційний внесок є проблемою](/uk/registrace/kdyz-je-poplatek-problem/).

## Крок 9 — Прибуття і перший день {#prijezd}

Подія триває з четверга до неділі. **Прибути можеш у четвер або п'ятницю**; більша частина учасників прибуває в четвер вдень або ввечері.

**Після прибуття:**

1. Паркування за вказівками організаторів (паркувальна картка).
2. Встановлення намету в таборі твоєї армії.
3. **Реєстрація на місці** — здаси згоду законного представника (якщо до 18), підтвердиш персональні дані, отримаєш орієнтаційні матеріали.
4. **Схвалення зброї і костюма** — перевіряє організатор; за костюмом тобі присвоять кількість життів (2–4).
5. Зустріч з армією, вечірнє вогнище, табірна атмосфера.



## Крок 10 — Вихідні в грі {#vikend}

**Четвер:** прибуття, побудова табору, перша реєстрація, вечірнє вогнище. *День спокою і знайомств.*

**П'ятниця:** гра в таборі, супровідна програма, дитяча гра, ярмарок, арени. *День, коли табір оживає.*

**Субота:** ранкове шикування, перевірка спорядження, розподіл життів, **головна гра і битва**, фінальний акорд гри. *Головний день.*

**Неділя:** збирання, прибирання табору, від'їзд. *Допомога з прибиранням вітається — лука після нас має виглядати як лука.*

## Словник {#slovnicek}

- **Організатор / орг** — людина, що готує подію і під час події вирішує спірні ситуації. Що скаже — те діє.
- **Сторона** — великий сюжетний блок. На події є дві основні сторони.
- **Армія** — конкретна ігрова група. Напр., Ґондор, Рохан, Ельфи, Гноми, Орки, Гарад, Умбар.
- **Володар / командир** — ігровий лідер армії або її частини — тримає групу разом.
- **NPC / неігровий персонаж** — персонаж, керований організаторами чи помічниками, що допомагає сюжету і грі.
- **Життя** — кількість влучань, які твій персонаж за правилами витримає.
- **Оживлення** — повернення в гру за правилами конкретної фази.
- **Схвалення зброї** — перевірка, чи зброя безпечна і придатна для гри.
- **Схвалення костюма** — перевірка, чи костюм відповідає атмосфері. За ним може бути визначена і кількість життів.
- **Небойовий супровід** — учасник, який у грі і в костюмі, але не б'ється.
- **Реєстраційний внесок** — внесок за участь. Після реєстрації отримаєш платіжні дані, і потрібно сплатити впродовж 10 днів.

## Куди далі {#kam-dal}

- [Правила і безпека](/uk/pravidla/) — бій, ураження, зброя, костюми
- [Армії та сторони](/uk/frakce/) — обери свою сторону та армію
- [Практична інформація](/uk/organizacni-informace/) — дата, місце, розклад, транспорт
- [Організаційна інформація](/uk/organizacni-informace/) — вік, документи, паркування, табір
- [Реєстрація](/uk/registrace/) — як зареєструватися, розмір внеску, знижки
- [Для батьків](/uk/pro-rodice/) — все про участь дітей
- [Дитяча гра](/uk/detska-hra/) — програма для маленьких пригодників
- [FAQ](/uk/faq/) — відповіді на найчастіші питання

## Рекомендоване читання {#cteni}

Корисні статті з [larp.cz](https://www.larp.cz/) — спільнотного порталу чеських LARPерів (переважно чеською). Інструкції з виготовлення костюмів, зброї та спорядження також на [hurka.uhobitu.cz](https://hurka.uhobitu.cz/) — магазин і туторіали Вацлава Хорського й майстерні B5A (чеською).

<div class="newcomer-info-grid">
  <section class="newcomer-info-card" aria-labelledby="reading-0-title">
    <h3 id="reading-0-title" class="newcomer-info-card-title">
      <span aria-hidden="true">◆</span>
      Для абсолютних новачків:
    </h3>
    <ul>
      <li><a href="https://www.larp.cz/?q=cs/clanek/506/o-larpu-pro-novacky">Про LARP для новачків</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/6788/pred-na-a-po-prvnim-larpu">До, під час і після першого LARPу</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/670/moje-prvni-drevarna">Моя перша „древарна"</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/657/role-play-aneb-ja-nejsem-ja">Рольова гра, або Я — не я</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/658/slovnicek-popisu-drevaren">Словник описів „древарень"</a></li>
    </ul>
  </section>

  <section class="newcomer-info-card" aria-labelledby="reading-1-title">
    <h3 id="reading-1-title" class="newcomer-info-card-title">
      <span aria-hidden="true">◆</span>
      Костюм:
    </h3>
    <ul>
      <li><a href="https://www.larp.cz/?q=cs/clanek/2623/jak-na-kostym-zaciname">Як на костюм — починаємо</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/680/uvod-do-vyroby-kostymu">Вступ до виготовлення костюма</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/6824/charakter-postavy-a-kostym">Характер персонажа і костюм</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/1435/vyroba-tuniky">Виготовлення туніки</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/1613/pokryvka-hlavy">Головний убір</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/3428/navod-na-vyrobu-kozene-capky">Інструкція до шкіряної шапки</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/1330/oblicej-v-barvach-duhy">Обличчя в кольорах веселки</a></li>
      <li><a href="https://hurka.uhobitu.cz/product.php?id_product=841">Jak na tvorbu kostýmu</a> <span class="text-xs">(hurka)</span></li>
      <li><a href="https://hurka.uhobitu.cz/product.php?id_product=842">Základní kostýmy pro inspiraci</a> <span class="text-xs">(hurka)</span></li>
      <li><a href="https://hurka.uhobitu.cz/product.php?id_product=846">Návod košile, tunika, kaftan</a> <span class="text-xs">(hurka)</span></li>
      <li><a href="https://hurka.uhobitu.cz/product.php?id_product=845">Návod kalhoty</a> <span class="text-xs">(hurka)</span></li>
      <li><a href="https://hurka.uhobitu.cz/product.php?id_product=843">Návod čepice</a> <span class="text-xs">(hurka)</span></li>
      <li><a href="https://hurka.uhobitu.cz/product.php?id_product=847">Návod krpce (obuv)</a> <span class="text-xs">(hurka)</span></li>
      <li><a href="https://hurka.uhobitu.cz/product.php?id_product=848">Návod ovinky, onuce</a> <span class="text-xs">(hurka)</span></li>
      <li><a href="https://hurka.uhobitu.cz/product.php?id_product=849">Návod žebradlo</a> <span class="text-xs">(hurka)</span></li>
    </ul>
  </section>

  <section class="newcomer-info-card" aria-labelledby="reading-2-title">
    <h3 id="reading-2-title" class="newcomer-info-card-title">
      <span aria-hidden="true">◆</span>
      Спорядження:
    </h3>
    <ul>
      <li><a href="https://www.larp.cz/?q=cs/clanek/6790/vybava-pro-%E2%80%9Ezivot%E2%80%9D-postavy">Спорядження для „життя" персонажа</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/1685/zavazadla">Багаж</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/8988/ackovy-stan-aneb-od-patlala-pro-patlaly">А-намет від аматора для аматорів</a></li>
      <li><a href="https://hurka.uhobitu.cz/product.php?id_product=844">Návod čutora</a> <span class="text-xs">(hurka)</span></li>
    </ul>
  </section>

  <section class="newcomer-info-card" aria-labelledby="reading-3-title">
    <h3 id="reading-3-title" class="newcomer-info-card-title">
      <span aria-hidden="true">◆</span>
      Зброя:
    </h3>
    <ul>
      <li><a href="https://www.larp.cz/?q=cs/clanek/1297/valecnik-a-jeho-mocny-mec">Воїн і його могутній меч</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/1149/vyroba-stitu">Виготовлення щита</a></li>
      <li><a href="https://www.larp.cz/?q=cs/clanek/4143/navod-na-hlavici-sipu">Інструкція до наконечника стріли</a></li>
      <li><a href="https://larp.cz/odkazy#navody">Інструкції на LARP.cz</a></li>
      <li><a href="https://hurka.uhobitu.cz/product.php?id_product=314">Návod na tesák</a> <span class="text-xs">(hurka)</span></li>
      <li><a href="https://hurka.uhobitu.cz/product.php?id_product=799">Video návod meč gotika</a> <span class="text-xs">(hurka)</span></li>
      <li><a href="https://hurka.uhobitu.cz/product.php?id_product=540">Bezpečnostní hlavice na šípy</a> <span class="text-xs">(hurka)</span></li>
    </ul>
  </section>
</div>
